 ...
 * <p>
 * ResoulverUtil 主要用于解析给定package目录下满足特定条件的class，
 * 从源码中可以看到，实际是调用VFS.getInstance().list(path) 解析
 *
 * <p>
...
public class ResolverUtil<T> {
   ...
   * 每一个class被发现时都会提供一个Test(验证器)，如果验证返回true，这个class会被保存起来。通过{@link #getClasses()}获取累计的classes。
   * <p>
