...
public class BeanWrapper extends BaseWrapper {
  ...
  public Object get(PropertyTokenizer prop) {
    // <1> 获得集合类型的属性的指定位置的值
    //<1> 处，获得集合类型的属性的指定位置的值。例如说：User 对象的 list[0] 。
  ...
  public Class<?> getGetterType(String name) {
    ...
    if (prop.hasNext()) {
      ...
      // 例如：richType.richMap.nihao ，其中 richMap 是 Map 类型，
      // 而 nihao 的类型，需要获得到 nihao 的具体值，才能做真正的判断。
      else {
  ...
  public MetaObject instantiatePropertyValue(String name, PropertyTokenizer prop, ObjectFactory objectFactory) {
    ...
    try {
      ...
      // <1> 设置当前对象的值
      //调试MetaObjectTest#shouldGetAndSetNestedMapPairUsingArraySyntax() 这个单元测试方法。
      set(prop, newObject);
    } catch (Exception e) {
  ...
  private Object getBeanProperty(PropertyTokenizer prop, Object object) {
    try {
      //获取object对象对应prop.getName()属性的GetInvoker
  ...
  public void add(Object element) {
    //方法都是不支持的，直接抛出 UnsupportedOperationException 异常
